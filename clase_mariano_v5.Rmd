...
evyth <- evyth %>%
  ...
  mutate(turistas=turistas/1000000)

#Gráfico

ggplot(data = evyth,aes(x=anio_mes,y=turistas))+
  ...
  labs(title = "Evolución del total de turistas",
...
ggplot(data = gpd,aes(x=anio, y=gasto_promedio_anual,
                     color=region_destino,label=region_destino))+ 
  geom_text()+
  labs(title = "Gasto promedio según región de destino.",
       subtitle = "GAU, 2012-2022.",
       x = "",
       y = "",
       colour="",
...
## *Localidades relevadas en la actualidad*

```{r fig.height = 6, fig.width=10, fig.align='center'}


arg <- read_sf("mapas/argentina.geojson")
locs <- read_sf("mapas/localidades_eoh.gpkg") 

ggplot()+
  geom_sf(data=arg,fill="grey")+
  theme_light()+
  theme_void()+
  theme(legend.position = "none")+
  ggrepel::geom_label_repel(data = locs, #Etiquetas de algunos pasos
                            aes(label = localidad_eoh,
                                geometry = geom),
                            stat = "sf_coordinates",
                            size = 4)
rm(arg,locs)
